...
def make_prediction(
    *, transaction: t.Union[pd.DataFrame, dict], identity: t.Union[pd.DataFrame, dict]
) -> dict:
    """Make a prediction using a saved model pipeline."""

    transaction_df = pd.DataFrame(transaction)
    identity_df = pd.DataFrame(identity)
    validated_data, errors = validate_inputs(
        transaction=transaction_df, identity=identity_df
    )
